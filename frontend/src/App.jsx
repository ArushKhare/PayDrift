...
const sumDrift = (rows) => rows.reduce((s, r) => s + (r.drift || 0), 0);
...
function App() {
  ...
  const fetchData = () => {
    ...
    Promise.all([
      ...
      .then(
        ([driftPayroll, driftAI, driftSaas, trendPayroll, trendAI, trendSaas, utilization]) => {
          setData({ driftPayroll, driftAI, driftSaas, trendPayroll, trendAI, trendSaas, utilization });
          setLoading(false);
        }
      )
      .catch((e) => {
          ...
          {categories.map((cat) => {
            ...
            return (
              ...
              >
                ...
                <p className={`text-xs mt-1 ${isUp ? "text-red-400/70" : "text-emerald-400/70"}`}>
              ...
              <tbody>
                {active.driftData.map((row, i) => {
                  ...
                  return (
                    ...
                    >
                      {active.groupCols.map((col) => (
                        <td key={col} className="px-6 py-3 text-zinc-300 font-medium">{row[col]}</td>
                      ))}
                      <td
                      ...
                      <td
                        className={`px-6 py-3 text-right font-semibold ${isUp ? "text-red-400" : "text-emerald-400"}`}
                        style={{ fontFamily: "'DM Mono', monospace" }}
                      >
                        {isUp ? "+" : ""}{fmt(row.drift)}
                      </td>
                      <td
                        className={`px-6 py-3 text-right ${isUp ? "text-red-400/80" : "text-emerald-400/80"}`}
                        style={{ fontFamily: "'DM Mono', monospace" }}
                      >
                ...
                <tbody>
                  {data.utilization.map((row, i) => {
                    ...
                    return (
                      ...
                      >
                        <td className="px-6 py-3 text-zinc-300 font-medium">{row.service}</td>
                        <td
                          className="px-6 py-3 text-right text-zinc-400"
                          style={{ fontFamily: "'DM Mono', monospace" }}
                        >
                          ...
                          <div className="flex items-center justify-end gap-3">
                            <div className="w-20 h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                              <div
                                ...
                                style={{
                                  width: `${barWidth}%`,
                                  background: row.flagged ? "#ef4444" : "#10b981",
                                }}
                              />
                            </div>
                            <span
                              className={`text-xs ${row.flagged ? "text-red-400" : "text-zinc-400"}`}
                              style={{ fontFamily: "'DM Mono', monospace" }}
                            >
                              {pct}%
                            </span>
